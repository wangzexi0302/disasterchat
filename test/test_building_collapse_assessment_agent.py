...
from app.agents.building_collapse_assessment_agent import BuildingCollapseAssessmentAgent
...
def test_building_collapse_assessment_agent():
    """测试ImpactAssessmentAgent类的基本功能"""
    # 创建MultiModalAgent实例
    agent = BuildingCollapseAssessmentAgent()
    ...
    test_messages = [
        {
            ...
            "content": [
                {
                    "type": "text",
                    "text": "请分析该地区的受灾情况，评估建筑物的受损情况。返回信息使用中文"
                },
                {
                    "type": "image",
    ...
    try:
        ...
        logger.info(f"非流式响应图片:\n{len(response['message']['image'])}")
    except Exception as e:
...
if __name__ == "__main__":
    ...
    test_building_collapse_assessment_agent()
    logger.info("测试完成。")