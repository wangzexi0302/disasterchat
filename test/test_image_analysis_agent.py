...
def test_image_analysis_agent():
    ...
    image_pre_path = os.path.join(root_path, "./demo_data/pre.png")
    image_post_path = os.path.join(root_path, "./demo_data/post.png")
    ...
    test_messages[0]['content'][0]['text'] = "请根据我给你的**受灾前**的遥感影像，评估**灾前**起始点A到终止点B是否可通行。返回信息使用中文。"
    try:
        response = agent.run(test_messages)
        logger.info(f"非流式响应内容:\n{response['content'][0]}")
    except Exception as e:
    ...
    logger.info("测试【路径计算工具：评估A到B是否可达】的非流式响应...")
    test_messages[0]['content'][0]['text'] = "请根据我给你的**受灾后**的遥感影像，评估**灾后**起始点A到终止点B是否可通行。返回信息使用中文。"
    try:
        response = agent.run(test_messages)
        logger.info(f"非流式响应内容:\n{response['content'][0]}")
    except Exception as e:
    ...
    test_messages[0]['content'][0]['text'] = "请根据我给你的受灾前后的遥感影像，完成以下3项任务：\n1. 评估建筑物的受损严重程度；\n2. 统计各类地物的受灾面积；\n3. 评估受灾后从A到B道路是否可通行。"
    try:
        response = agent.run(test_messages)
        logger.info(f"非流式响应内容:\n{response['content'][0]}")
    except Exception as e:
